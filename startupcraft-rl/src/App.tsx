...
import { INITIAL_AGENTS, SCENARIOS } from './constants';
  ...
  return (
    <div className={`h-screen w-screen flex flex-col font-sans select-none ${isLightMode ? 'bg-slate-50 text-slate-900' : 'bg-slate-900 text-slate-200'}`}>
      ...
      <main className="flex-1 flex overflow-hidden">
        {/* Left Sidebar: Agents */}
        <motion.aside 
          animate={{ width: isAgentsOpen ? 320 : 40 }}
        ...
        >
          <div 
            className={`p-3 border-b flex items-center cursor-pointer transition-colors h-12 ${isLightMode ? 'border-slate-200 hover:bg-slate-100' : 'border-slate-800 hover:bg-slate-800'}`}
            onClick={() => setIsAgentsOpen(!isAgentsOpen)}
          >
            <ChevronRight size={14} className={`text-slate-500 transition-transform shrink-0 ${isAgentsOpen ? 'rotate-180' : ''}`} />
            <h2 className={`ml-2 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] whitespace-nowrap transition-opacity ${!isAgentsOpen ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'}`}>Agents (RL Entities)</h2>
          </div>
          <div className="p-4 overflow-y-auto custom-scrollbar w-80">
        ...
        <section className={`flex-1 flex flex-col relative ${isLightMode ? 'bg-slate-50' : 'bg-slate-900'}`}>
          ...
          <div className="flex-1 relative overflow-hidden flex items-center justify-center p-8 bg-green-200">
            ...
            {viewMode === '3d' ? (
              ...
              <motion.div 
                className="relative w-full max-w-6xl aspect-[2.5/1] bg-slate-200 rounded-xl border-[12px] border-slate-700 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden z-10"
                animate={{ 
                  scale: zoomLevel,
                  rotateX: 0,
                  y: 0
              ...
              {/* Lobby Area (Left 20%) */}
              <div className="absolute left-0 top-0 w-[20%] h-full bg-slate-300/40">
                <div className="absolute top-4 left-4 text-slate-500 font-bold tracking-widest text-xs opacity-50">LOBBY</div>
                
                {/* Arcade Machine */}
                <div className="absolute left-4 top-4 w-10 h-12 bg-purple-900 rounded-sm shadow-lg border-2 border-purple-950 flex flex-col items-center justify-start overflow-hidden">
                ...
                {/* Reception Desk */}
                <div className="absolute left-10 top-1/2 -translate-y-1/2 w-8 h-32 bg-amber-800 rounded-sm shadow-xl border-2 border-amber-950">
                ...
                {/* Sofa */}
                <div className="absolute left-4 bottom-24 w-16 h-10 bg-indigo-700 rounded-lg shadow-xl border-2 border-indigo-900 flex items-center justify-center">
                  <div className="w-14 h-8 bg-indigo-600 rounded-md" />
                </div>
                
                {/* Plant */}
                <div className="absolute left-4 bottom-8 w-10 h-10 bg-emerald-600 rounded-full shadow-xl border-2 border-emerald-900 flex items-center justify-center">
                  <div className="absolute inset-1 bg-emerald-500 rounded-full" />
                  <div className="w-4 h-4 bg-emerald-400 rounded-full" />
                </div>
              </div>

              {/* Grid of 8 rooms (Right 80%) */}
              {agents.map((agent, i) => {
                const col = i % 4;
                ...
                return (
                  <div key={`room-${i}`} className={`absolute ${roomBg} overflow-hidden shadow-inner`}
                     ...
                     <div className={`absolute ${deskColor} rounded-sm shadow-lg border border-black/30`}
                          style={{
                            left: '10%', width: '80%', height: '25%',
                            top: row === 0 ? '15%' : '60%'
                          }}>
                        {/* Monitor */}
                        <div className="absolute bg-slate-900 w-10 h-4 left-1/2 -translate-x-1/2 rounded-sm shadow-md"
                             style={{ 
                               top: row === 0 ? '60%' : '10%',
                               borderTop: row === 0 ? 'none' : '2px solid #38bdf8',
                               borderBottom: row === 0 ? '2px solid #38bdf8' : 'none'
                             }} />
                     </div>
                     {/* Chair */}
                     <div className="absolute bg-slate-800 w-7 h-7 rounded-full left-1/2 -translate-x-1/2 shadow-lg border-2 border-slate-900"
                          style={{ top: '45%' }} />
                  </div>
                )
              })}
                ...
                if (agent.status === 'idle') {
                  if (agent.talkingTo) {
                    ...
                    if (targetIndex !== -1 && targetIndex !== i) {
                      ...
                      targetX = tCenterX + (i % 2 === 0 ? -3 : 3);
                      targetY = tRow === 0 ? tChairY + 15 : tChairY - 15;
                      rotate = tRow === 0 ? 0 : 180;
                    } else {
                      targetX = 8 + (i % 3) * 3;
                      targetY = 25 + (i * 7);
        ...
        {/* Right Sidebar: Logs */}
        <motion.aside 
          animate={{ width: isLogOpen ? 320 : 40 }}
        ...
        >
          ...
          >
            <ChevronRight size={14} className={`text-slate-500 transition-transform shrink-0 ${isLogOpen ? '' : 'rotate-180'}`} />
            <h2 className={`text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] whitespace-nowrap ${!isLogOpen && 'opacity-0'}`}>Simulation Log</h2>
            <MessageSquare size={14} className={`text-slate-500 shrink-0 ${!isLogOpen && 'opacity-0'}`} />
          </div>
          <div className={`flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar w-80 ${isLightMode ? 'bg-slate-100/50' : 'bg-slate-950/30'}`}>
            {logs.map(log => (
          ...
          <div className={`p-4 border-t w-80 ${isLightMode ? 'bg-slate-100 border-slate-200' : 'bg-slate-950/50 border-slate-800'}`}>
